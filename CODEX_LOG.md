...
### Iterazione 2026-08-16 (piano recognizer reale — approvato, non ancora implementato)
- Sessione di planning (plan mode) a partire da `prompt_recognizer.md`: piano per sostituire `MockMonumentRecognizer` con un recognizer reale (image embedding + cosine similarity), rispettando l'interfaccia `MonumentRecognizer` e senza toccare `ScanController`.
- Esplorazione completa della pipeline esistente: interfaccia recognizer, `ScanController` (throttle/busy/streak/lock già validati, letti ma non modificati), modello dati `Monument`/`MonumentRecognitionProfile` (campo `embeddings` presente ma vuoto), `RecognitionAssetCache` (codice morto, nessun chiamante). Verificato che la camera usa `ResolutionPreset.medium` (`camera_preview_controller.dart`): i JPEG passati al recognizer sono ~720×480, non full-res come inizialmente ipotizzato — semplifica il preprocessing (niente isolate per la decodifica, solo `dart:ui` nativo).
- **Decisioni prese e approvate:**
  - Modello di embedding: **MobileNetV3 feature vector** come baseline da misurare (non il "migliore possibile" a tavolino — l'unico motivo valido per scegliere altro è una misura che lo dimostri necessario).
  - Runtime: **`tflite_flutter: 0.12.1`** (pin esatto), preferito a `flutter_litert` dopo review incrociata — publisher ufficiale TensorFlow, più stabile per un pilota, a costo di un possibile bump `minSdk 24 → 26` (da verificare, **approvato condizionalmente**) e setup manuale delle native lib. Scelta reversibile a costo minimo: il package è confinato in un solo file.
  - Soglia e streak: **da misurare**, non da assumere — protocollo di calibrazione con split reference/probe per sessione, negativi hard + generici, FAR/FRR, streak derivato dai tassi per-frame misurati.
  - Foto dei monumenti: **alcune già disponibili** (non zero come da stato iniziale del repo), ma da recuperare dal telefono — trasferimento manuale concordato con l'utente, non ancora avvenuto.
